...
useEffect(() => {
  const fetchRequests = async () => {
    try {
      ...
      setRequests(Array.isArray(data.inventoryRequests) ? data.inventoryRequests : Array.isArray(data) ? data : []);
    } catch (error) {
    ...
    <ul >
      {requests &&requests.map((request) => (
        <li className="gameRequestCard" key={request.id}>
          ...
          <p>Day(s): {request.details || "N/A"}</p>
          <p>Email: {request.email || "N/A"}</p>
          <p>Discord: {request.discordName || "N/A"}</p> 
        </li>
      ))}
